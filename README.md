# TechnoHistoria-Asiatica
THA (TecnoHistória Asiática) é um site simples sobre a história asiática em um curto período

## Atas:

# Atas de Reunião - Projeto Comércio e Circulação de Tecnologia

## 1 de junho de 2026
**Ata 1**

* **Participantes presentes:** *
  *Kauã Dias*
  * *Bryan Henrique*
  * *David dos Santos*
  * *Heitor Lisboa*
  * *Gustavo schmitz*
  * *Davi Lucas*

### **Anotações**

* **Tema:** Comércio e circulação de tecnologia
* **Cargos:**
  * **Líder:** *Gustavo*
  * **Secretário:** *Kauã*
  * **Professores do Projeto:** *Ivan* e *Mehran*

* **Responsabilidades:**
  * *Kauã Dias*: Busca por inspirações em outros sites e pesquisou paletas de cores para o site
  * *Bryan Henrique*: Realiza o rascunho da página no Figma
  * *David dos Santos*: Ajuda no Figma
  * *Heitor Lisboa*: Pesquisa informações históricas do tema
  * *Gustavo schmitz*: Busca por fontes de pesquisa para o projeto
  * *Davi Lucas*: Realiza a pesquisa de possíveis civilizações que podem ser utilizadas no projetos

* **Ações realizadas:**
  * Decisão do tema
  * Decisão do Líder e Secretário
  * Pesquisa sobre civilizações que podem ser utilizadas
  * Pesquisa da paleta de cores

* **Civilizações ou Regiões que podem ser usadas:**
  * Mesopotâmia
  * Egito Antigo
  * China Antiga
  * Grécia
  * Roma
  * África (provavelmente)
  * América (as três regiões)

---

## 8 de junho de 2026
**Ata 2**

* **Participantes presentes:** * *Kauã Dias*
  * *Bryan Henrique*
  * *David dos Santos*
  * *Heitor Lisboa*
  * *Gustavo schmitz*

* **Participantes ausentes:**
  * *Davi Lucas*

### **Anotações**

* **Tema:** Comércio e circulação de tecnologia
* **Cargos:**
  * **Líder:** *Gustavo*
  * **Secretário:** *Kauã*
  * **Professores do Projeto:** *Ivan* e *Mehran*

* **Responsabilidades:**
  * *Kauã Dias*: Decidiu a paleta de cores
  * *Bryan Henrique*: Fez o wireframe
  * *David dos Santos*: Auxiliou na criação do wireframe
  * *Heitor Lisboa*: Auxiliou na pesquisa
  * *Gustavo schmitz*: Realizou a pesquisa

* **Ações realizadas:**
  * Decisão da paleta de cores
  * Pesquisa de modelos de sites
  * Início do Figma
  * Decisão dos tópicos históricos

* **Modelos de sites:**
  * https://asia.si.edu/

---

## 15 de junho de 2026
**Ata 3**

* **Participantes presentes:** * *Kauã Dias*
  * *Bryan Henrique*
  * *David dos Santos*
  * *Davi Lucas*
  * *Gustavo schmitz*

* **Participantes ausentes:**
  * *Heitor Lisboa*

### **Anotações**

* **Tema:** Comércio e circulação de tecnologia
* **Cargos:**
  * **Líder:** *Gustavo*
  * **Secretário:** *Kauã*
  * **Professores do Projeto:** *Ivan* e *Mehran*

* **Responsabilidades:**
  * *Kauã Dias*: Começou a criação da logo
  * *Bryan Henrique*: consertou erros no wireframe
  * *David dos Santos*: Auxiliou na finalização do wireframe
  * *Davi Lucas*: Pesquisa de vários artigos e vídeos
  * *Gustavo schmitz*: Github, Pesquisa de artigos

* **Ações realizadas:**
  * Início da criação da logo
  * Pesquisa de artigos e vídeos
  * Início do github

---

## 22 de junho de 2026
**Ata 4**

* **Participantes presentes:**
   * *Bryan Henrique*
  * *David dos Santos*
  * *Gustavo schmitz*
  * *Heitor Lisboa*

* **Participantes ausentes:**
  * *Davi Lucas*
  * *Kauã Dias* com justificativa

### **Anotações**

* **Tema:** Comércio e circulação de tecnologia
* **Cargos:**
  * **Líder:** *Gustavo*
  * **Secretário:** *Kauã*
  * **Professores do Projeto:** *Ivan* e *Mehran*

* **Responsabilidades:**
  * *Bryan Henrique*: consertos técnicos do wireframe
  * *David dos Santos*: Auxiliou na finalização do wireframe
  * *Gustavo schmitz*: Github e substituição do responsável da ata (*Kauã Dias*)
  * *Heitor Lisboa*: Pesquisa

* **Ações realizadas:**
  * Organização do Github e ata
  * Pesquisa sobre tópicos (China, Índia etc)
  * Alterações no wireframe (nova ideia de design)

---

## 6 de julho de 2026
**Ata 5**

* **Participantes presentes:**
   * *Bryan Henrique*
  * *David dos Santos*
  * *Gustavo schmitz*
  * *Heitor Lisboa*
  * *Davi Lucas*
  * *Kauã Dias*

 
* **Participantes ausentes:**

### **Anotações**

**O professor Kennedy aceitou ser o coaborador do projeto**

* **Tema:** Comércio e circulação de tecnologia
* **Cargos:**
  * **Líder:** *Gustavo*
  * **Secretário:** *Kauã*
  * **Professores do Projeto:** *Ivan* e *Mehran*
  * **Colaborador do projeto:** *Kennedy Araujo*

* **Responsabilidades:**
  * *Bryan Henrique*: Auxílio na pesquisa (ajudou na pesquisa após finalizar suas tarefas anteriores)
  * *David dos Santos*: Auxílio na pesquisa (ajudou na pesquisa após finalizar suas tarefas anteriores)
  * *Gustavo schmitz*: realização da ata (uma substituição temporaria) e pesquisando as convenções de arquivos do projeto
  * *Heitor Lisboa*: Encarregado da pesquisa
  * *Davi Lucas*: Encarregado da pesquisa
  * *Kauã Dias*: Finalizando wireframe

* **Ações realizadas:**
  * Continuando a pesquisa sobre tópicos (China, Índia etc)
  * Finalizando o wireFrame no novo modelo

---

## 13 de julho de 2026
**Ata 6**

* **Participantes presentes:**
   * *Bryan Henrique*
  * *David dos Santos*
  * *Gustavo schmitz*
  * *Heitor Lisboa*
  * *Davi Lucas*
  * *Kauã Dias*

 
* **Participantes ausentes:**

### **Anotações**

**O professor Mehran avaliou todos os integrantes de cada grupo**

* **Tema:** Comércio e circulação de tecnologia
* **Cargos:**
  * **Líder:** *Gustavo*
  * **Secretário:** *Kauã*
  * **Professores do Projeto:** *Ivan* e *Mehran*
  * **Colaborador do projeto:** *Kennedy Araujo*

* **Responsabilidades:**
  * *Bryan Henrique*: Organizou o modelo básico do figma e auxiliou na organização da pesquisa
  * *David dos Santos*: Finalizou o figma
  * *Gustavo schmitz*: Criou a raiz de arquivos do Github
  * *Heitor Lisboa*: Organizou o documento da pesquisa no google docs
  * *Davi Lucas*: Organizou e filtrou a pesquisa
  * *Kauã Dias*: Criou a ata de reunião e Finalizou o Figma e criou as redes sociais

* **Ações realizadas:**
  * Finalização do Figma
  * Organização da pesquisa e filtro da pesquisa
  * Criação das redes sociais
  * Inicio da criação do HTML

---

## 3 de Agosto de 2026
**Ata 7**

* **Participantes presentes:**
   * *Bryan Henrique*
  * *David dos Santos*
  * *Gustavo schmitz*
  * *Heitor Lisboa*
  * *Davi Lucas*
  * *Kauã Dias*

 
* **Participantes ausentes:**

### **Anotações**

**Reorganização pós recesso**

* **Tema:** Comércio e circulação de tecnologia
* **Cargos:**
  * **Líder:** *Gustavo*
  * **Secretário:** *Kauã*
  * **Professores do Projeto:** *Ivan* e *Mehran*
  * **Colaborador do projeto:** *Kennedy Araujo*

* **Responsabilidades:**
  * *Bryan Henrique*: Página HTML individual e procurar arquivos para os assets
  * *David dos Santos*: Página HTML individual e procurar arquivos para os assets
  * *Gustavo schmitz*: Página HTML individual e pesquisa sobre necessidades (ensinar git ao grupo e o JS para o Quiz)
  * *Heitor Lisboa*: Página HTML individual e organizar como o conteúdo será inserido no site 
  * *Davi Lucas*: Página HTML individual e organizar como o conteúdo será inserido no site
  * *Kauã Dias*:Página HTML individual e organizar como o conteúdo será inserido no site
    
* **Ações realizadas:**
 * Início do HTML
 * divisão de tarefas no desenvolvimento
 * Logo recriada como arquivo (só falta converter para SVG)
  
---
## 10 de Agosto de 2026
**Ata 8**

* **Participantes presentes:**
   * *Bryan Henrique*
  * *David dos Santos*
  * *Gustavo schmitz*
  * *Davi Lucas*
  * *Kauã Dias*

 
* **Participantes ausentes:**
   * *Heitor Lisboa* sem justificativa


### **Anotações**

**Reformulação das imagens, wireframe e logo e continuação do HTML**

* **Tema:** Comércio e circulação de tecnologia
* **Cargos:**
  * **Líder:** *Gustavo*
  * **Secretário:** *Kauã*
  * **Professores do Projeto:** *Ivan* e *Mehran*
  * **Colaborador do projeto:** *Kennedy Araujo*

* **Responsabilidades:**
  * *Bryan Henrique*: feito a logo em svg no canva
  * *David dos Santos*: Continuação da página HTML individual
  * *Gustavo schmitz*: Continuação da página HTML individual e ensinou as necessidades (ensinar git e github CLI ao grupo e o JS para o Quiz)
  * *Davi Lucas*: Continuação da página HTML individual 
  * *Kauã Dias*: Continuação da página HTML individual e realização da ata
    
* **Ações realizadas:**
 * Continuação do HTML
 * Estudar git e github
 * Reformulação da logo e imagens do site
  
---
## 17 de Agosto de 2026
**Ata 9**

* **Participantes presentes:**
   * *Bryan Henrique*
  * *David dos Santos*
  * *Gustavo schmitz*
  * *Davi Lucas*
  * *Kauã Dias*
  * *Heitor Lisboa* 


### **Anotações**

**Fotografia dos integrantes do grupo, finalização do HTML e inicio do CSS**

* **Tema:** Comércio e circulação de tecnologia
* **Cargos:**
  * **Líder:** *Gustavo*
  * **Secretário:** *Kauã*
  * **Professores do Projeto:** *Ivan* e *Mehran*
  * **Colaborador do projeto:** *Kennedy Araujo*

* **Responsabilidades:**
  * *	•	Bryan Henrique: Finalizou as mudanças do wireframe, finalizou o HTML e prosseguiu para o CSS;
	* * •	David dos Santos: Finalização do HTML e início do CSS;
	* * •	Gustavo schmitz: Pesquisa de imagens para a página de destaques e substituiu a página línguas;
	* * •	Davi Lucas: Finalização do HTML e início do CSS;
	* * •	Kauã Dias: Finalização do HTML, início do CSS, fotografou os integrantes;
	* * •	Heitor Lisboa: Finalização do HTML e início do CSS

    
* **Ações realizadas:**
 * Finalização do HTML
 * Início do CSS
 * Fotografias dos integrantes para site
  
---
## 24 de Agosto de 2026
**ata 10**

* **Participantes presentes:**
   * *Bryan Henrique*
  * *David dos Santos*
  * *Gustavo schmitz*
  * *Davi Lucas*
  * *Kauã Dias*
  * *Heitor Lisboa* 

### **Anotações**

**Finalização do CSS, Organização do Github e Realização do documento individual de realizações no segundo trimestre**
* **Tema:** Comércio e circulação de tecnologia
* * **Cargos:**
  * **Líder:** *Gustavo*
  * **Secretário:** *Kauã*
  * **Professores do Projeto:** *Ivan* e *Mehran*
  * **Colaborador do projeto:** *Kennedy Araujo*

* **Responsabilidades:**
  * *	•	Bryan Henrique: Inicio do CSS e criou o documento individual;
	* * •	David dos Santos: Criou o documento individual;
	* * •	Gustavo schmitz: Criou o documento individual;
	* * •	Davi Lucas: Finalizou o CSS, enviou os arquivos pro GitHub e Criou o documento individual;
	* * •	Kauã Dias: Organização do GitHub, realização do documento individual e organização das atas de reunião;
	* * •	Heitor Lisboa: Finalizou o HTML, iniciou o CSS e Criou o documento individual.
